...
describe('CodebaseDocsAIClient', () => {
  ...
  it('passes run list source role filters through the HTTP API while preserving sanitization', async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      jsonResponse({
        runs: [
          {
            id: 'run_backend',
            name: `Backend Docs ${rawOpenAiKey} .env SHOULD_NOT_APPEAR`,
            status: 'completed',
            sources: [
              {
                name: `Backend ${rawStoragePath}`,
  ...
  it('rejects invalid run list source role filters without network requests or raw value exposure', async () => {
    ...
    try {
      await client.documentationRuns.list({ role: rawRole as never });
      throw new Error('Expected list to reject invalid source role.');
    } catch (error) {
      const payload = JSON.stringify(error);
      expect(error).toBeInstanceOf(CodebaseDocsAIClientError);
      expect((error as CodebaseDocsAIClientError).status).toBe(0);
      expect((error as CodebaseDocsAIClientError).code).toBe('RUN_LIST_SOURCE_ROLE_INVALID');
      expect((error as CodebaseDocsAIClientError).message).toBe(
  ...
  it('passes run list updated-at filters through the HTTP API while preserving sanitization', async () => {
    const rawOpenAiKey = `sk-${'u'.repeat(24)}`;
    const rawStoragePath = `/private/tmp/codebase-docs-ai/${rawOpenAiKey}/.env/SHOULD_NOT_APPEAR/run.json`;
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      jsonResponse({
        runs: [
          {
            ...
            name: `Updated Range Docs ${rawOpenAiKey} .env SHOULD_NOT_APPEAR`,
            status: 'completed',
            sources: [
              {
                name: `Backend ${rawStoragePath}`,
