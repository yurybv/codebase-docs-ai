...
### 2026-05-30: Phase 128 SDK Run Listing Limit Option

- Added a `documentationRuns.list({ limit })` SDK option that maps to the API run listing limit query.
- Added SDK-side validation for list limits before network requests, with sanitized stable invalid-limit errors.
- Added SDK regression coverage proving limited list responses remain sanitized and invalid secret-bearing limit values are not exposed.
...
### 2026-05-30: Phase 129 Web Run Listing Limit Control

- Added a Web run history limit selector that sends the API `limit` query during manual refresh.
- Kept the run history view as a thin API-backed operator surface with no analysis, rendering, or cleanup logic.
- Added Web regression coverage for selected limit requests, sanitized limited summaries, and sanitized run-list limit API errors.
...
### 2026-05-30: Phase 132 SDK Run Listing Status Filter Option

- Added a `documentationRuns.list({ status })` SDK option that maps to the API run listing status query.
- Added SDK-side validation for list status filters before network requests, with sanitized stable invalid-status errors.
- Added SDK regression coverage proving status-filtered list responses remain sanitized and invalid secret-bearing status values are not exposed.
...
### 2026-05-30: Phase 137 Web Run Listing Source Role Filter Control

- Added a Web run history source role filter selector that sends the API `role` query during manual refresh.
- Kept the run history view as a thin API-backed operator surface with no analysis, rendering, or cleanup logic.
- Added Web regression coverage for selected role requests, sanitized filtered summaries, and sanitized run-list role API errors.
...
### 2026-05-30: Phase 144 API Run Listing Updated-At Range Filter Contract

- Added API `updatedAfter` and `updatedBefore` query filters for `GET /v1/documentation-runs`.
- Validated updated-at filter values as ISO timestamps before run storage listing, with sanitized `RUN_LIST_UPDATED_AFTER_INVALID` and `RUN_LIST_UPDATED_BEFORE_INVALID` errors.
- Added service and HTTP regression coverage proving updated-at filtered summaries and invalid secret-bearing timestamp inputs remain sanitized.
...
### 2026-05-30: Phase 149 Web Run History Name Search Control And Audit

- Added a Web run history name search control that forwards the API `name` filter.
- Preserved selected limit, status, source role, name, updated-at range, and cursor values across Web run history pagination.
- Added Web regression coverage for sanitized name-filtered list output and sanitized name API errors.
...
### 2026-05-30: Phase 150 Run Listing Name Search Cross-Surface Regression Audit

- Audited API, SDK, CLI, and Web run-name search filtering as one public operator contract.
- Tightened API service and HTTP coverage so name filtering composes with limit, status, role, updated-at range, and cursor pagination.
- Tightened SDK and CLI list coverage so name-filtered requests preserve selected status, role, updated-at range, and cursor values without exposing raw secret-bearing values.
...
### 2026-05-30: Phase 153 Web Run History Output Format Filter Control And Audit

- Added a Web run history output format selector that forwards the API `format` filter.
- Preserved selected limit, status, source role, name, output format, updated-at range, and cursor values across Web run history pagination.
- Added Web regression coverage for sanitized format-filtered list output and sanitized format API errors.
...
### 2026-05-30: Phase 159 API Run Listing Created-At Range Filter Contract

- Added API `createdAfter` and `createdBefore` query filters for `GET /v1/documentation-runs`.
- Validated created-at filter values as ISO timestamps before run storage listing, with sanitized `RUN_LIST_CREATED_AFTER_INVALID` and `RUN_LIST_CREATED_BEFORE_INVALID` errors.
- Added service and HTTP regression coverage proving created-at filtered summaries and invalid secret-bearing timestamp inputs remain sanitized.
...
### 2026-05-30: Phase 162 Run Listing Created-At Cross-Surface Regression Audit

- Audited API, SDK, CLI, and Web created-at filtering as one public operator contract.
- Tightened API service and HTTP coverage so created-at filtering composes with limit, status, role, name, format, source-count range, updated-at range, and cursor pagination.
- Kept SDK, CLI, and Web created-at filtered list coverage aligned with sanitized output and sanitized invalid timestamp errors.
...
### 2026-05-30: Phase 163 API Run Listing Sort Direction Contract

- Added an API `sort` query parameter for `GET /v1/documentation-runs`, supporting `updatedAt:desc` and `updatedAt:asc`.
- Preserved `updatedAt:desc` as the default and made cursor pagination follow the selected sort direction deterministically.
- Added service and HTTP regression coverage for sorted list pages and invalid secret-bearing sort values.
...
### 2026-05-30: Phase 165 Web Run History Sort Direction Control And Audit

- Added a Web run history sort selector for `updatedAt:desc` and `updatedAt:asc`.
- Preserved selected limit, status, source role, name, output format, source-count range, sort direction, created-at range, updated-at range, and cursor values across Web run history pagination.
- Added Web regression coverage for sanitized sorted list output and sanitized sort API errors.
...
### 2026-05-30: Phase 169 Web Run History Created-At Sort Control And Audit

- Extended the Web run history sort selector to include `createdAt:desc` and `createdAt:asc`.
- Preserved selected limit, status, source role, name, output format, source-count range, created-at sort, created-at range, updated-at range, and cursor values across Web run history pagination.
- Updated Web regression coverage for created-at sorted list requests while preserving sanitized history output and sort error coverage.
...
### 2026-05-30: Phase 170 Run Listing Created-At Sort Cross-Surface Regression Audit

- Audited API, SDK, CLI, and Web created-at sort behavior as one public operator contract.
- Tightened API service and HTTP coverage so `createdAt:asc` composes with limit, status, role, name, format, source-count range, created-at range, updated-at range, and cursor pagination.
- Kept SDK, CLI, and Web created-at sorted list coverage aligned with sanitized output and sanitized invalid sort errors.
...
### 2026-05-30: Phase 172 SDK And CLI Run Listing Completed-At Filters

- Added `completedAfter` and `completedBefore` options to the SDK `documentationRuns.list` helper and CLI `list-runs` command.
- Added SDK and CLI preflight completed-at validation, with sanitized stable invalid completed-at failures before network requests.
- Added SDK and CLI regression coverage for completed-at filtered list requests and secret-bearing invalid timestamp values.
...
### 2026-05-30: Phase 174 Run Listing Completed-At Cross-Surface Regression Audit

- Audited API, SDK, CLI, and Web completed-at filtering as one public operator contract.
- Tightened API service and HTTP coverage so completed-at filtering composes with limit, status, role, name, format, source-count range, sort, created-at range, updated-at range, and cursor pagination.
- Kept SDK, CLI, and Web completed-at filtered list coverage aligned with sanitized output and sanitized invalid timestamp errors.
...
### 2026-05-30: Phase 180 SDK And CLI Terminal Duration Surface Follow-Up

- Preserved API-provided terminal `durationMs` metadata through SDK `documentationRuns.list` results and CLI `list-runs` output without duplicating duration computation.
- Added SDK and CLI regression coverage proving duration-bearing summaries remain sanitized.
- Updated SDK and operations documentation for duration-bearing list summaries.
...
### 2026-05-30: Phase 182 Terminal Duration Cross-Surface Regression Audit

- Audited API, SDK, CLI, and Web terminal duration behavior as one public operator contract.
- Tightened API service and HTTP coverage so duration metadata composes with status, role, name, format, source-count, created-at, completed-at, updated-at, completed-at sort, and cursor pagination.
- Tightened SDK, CLI, and Web regression coverage so duration-bearing summaries remain sanitized across all-filter requests and paginated Web history.
...
### 2026-05-30: Phase 184 SDK And CLI Run Listing Duration Sort Option

- Extended SDK `documentationRuns.list({ sort })` and CLI `list-runs --sort` to accept `durationMs:desc` and `durationMs:asc`.
- Kept SDK and CLI sort validation local to callers before network requests.
- Updated SDK, CLI, README, and operations documentation for duration sort usage.
...
### 2026-05-30: Phase 186 Run Listing Duration Sort Cross-Surface Regression Audit

- Audited API, SDK, CLI, and Web duration sort behavior as one public operator contract.
- Tightened API service and HTTP coverage so `durationMs:desc` composes with status, role, name, format, source-count range, created-at range, completed-at range, updated-at range, and cursor pagination.
- Kept SDK, CLI, and Web duration-sorted list coverage aligned with sanitized output and sanitized invalid sort errors.
...
### 2026-05-31: Phase 187 API Run Listing Source-Count Sort Contract

- Added API `sourceCount:desc` and `sourceCount:asc` run listing sort options using safe public source-count summary metadata.
- Included `sourceCount` in newly issued cursors so source-count sorted pagination remains deterministic across first-page and cursor-page requests.
- Added service and HTTP regression coverage for source-count sorted list pages and sanitized invalid sort handling without exposing secret-bearing run names or storage paths.
